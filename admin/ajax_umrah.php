...
if ($method === 'GET') {
    if ($action === 'view_details') {
        ...
        if ($workflowId > 0) {
            ...
            // 1) workflow_step_id المباشر إن كان صحيحاً
            // 2) مطابقة status_id
            // 3) default_status_id الخاص بسير العمل
            // 4) أول خطوة initial
            // 5) أول خطوة في الترتيب
            $currentStepId = 0;
            $currentStep = null;
            $stepsById = [];
            foreach ($umrahWf['allSteps'] as $stp) {
                $stepsById[(int)$stp['id']] = $stp;
            }

            $directStepId = (int)($p['workflow_step_id'] ?? 0);
            if ($directStepId > 0 && isset($stepsById[$directStepId])) {
                $currentStepId = $directStepId;
                $currentStep = $stepsById[$directStepId];
            }

            if ($currentStepId <= 0) {
                $curStatus = (int)($p['status_id'] ?? 0);
                foreach ($umrahWf['allSteps'] as $stp) {
                    if ((int)($stp['status_id'] ?? 0) === $curStatus) {
            ...
            if ($currentStepId <= 0) {
                $defaultWorkflowStatusId = (int)($umrahWf['workflow']['default_status_id'] ?? 0);
                if ($defaultWorkflowStatusId > 0) {
                    foreach ($umrahWf['allSteps'] as $stp) {
                        if ((int)($stp['status_id'] ?? 0) === $defaultWorkflowStatusId) {
                            $currentStepId = (int)$stp['id'];
                            $currentStep = $stp;
                            break;
                        }
            ...
            if ($currentStepId <= 0 && !empty($umrahWf['allSteps'][0])) {
                $currentStepId = (int)$umrahWf['allSteps'][0]['id'];
                $currentStep = $umrahWf['allSteps'][0];
            }

            // توثيق الربط المباشر مع المرحلة لتفادي ضياع الانتقالات في الزيارات القادمة.
            if ($currentStepId > 0 && ((int)($p['workflow_step_id'] ?? 0) !== $currentStepId || (int)($p['workflow_id'] ?? 0) !== $workflowId)) {
                try {
                    $pdo->prepare("UPDATE passports SET workflow_id = ?, workflow_step_id = ? WHERE id = ?")->execute([$workflowId, $currentStepId, $id]);
                } catch (Throwable $e) { /* ignore */ }
            }
            $umrahWf['currentStepId'] = $currentStepId;
            $umrahWf['currentStep'] = $currentStep;
        ...
        if (empty($umrahWf['transitions']) && empty($umrahWf['currentStep']) && !$curIsFinal) {
            ...
            if (!$umrahWf['canEditWf']) {
                echo '<div class="alert alert-warning small py-2 text-center">تم إظهار الانتقالات المتاحة للقراءة فقط. تنفيذ الانتقال يتطلب صلاحية تعديل سير العمل.</div>';
            }
            ...
            foreach ($umrahWf['transitions'] as $tr) {
                ...
                $disabled = $umrahWf['canEditWf'] ? '' : ' disabled';
...
if ($method === 'POST') {
    ...
    if ($action === 'add_umrah') {
        try {
            ...
            // اربط المعاملة الجديدة مباشرةً بسير العمل الافتراضي وخطوته الأولى إن وُجدت
            ...
            $default_workflow_step_id = null;
            if (isset($settings['umrah_workflow_enabled']) && $settings['umrah_workflow_enabled']) {
                $default_workflow_id = get_workflow_id_by_transaction_type($pdo, 'umrah');
                if ($default_workflow_id) {
                    $stmtDefaultStep = $pdo->prepare("
                        SELECT id, status_id
                        FROM workflow_steps
                        WHERE workflow_id = ?
                        ORDER BY is_initial DESC, sort_order ASC, id ASC
                        LIMIT 1
                    ");
                    $stmtDefaultStep->execute([$default_workflow_id]);
                    $defaultStep = $stmtDefaultStep->fetch(PDO::FETCH_ASSOC);
                    if ($defaultStep) {
                        $default_workflow_step_id = (int)$defaultStep['id'];
                        if (!empty($defaultStep['status_id'])) {
                            $default_status_id = (int)$defaultStep['status_id'];
                        }
                    }
                }
            }
            ...
            // Extract customer_id and agent_id — always read from POST for all delivery types
            // (the user may have explicitly chosen a customer/agent even for cash/bank_transfer)
