...
// Check if a custom template exists for payment vouchers
$custom_template = null;
try {
    $stmt = $pdo->prepare("SELECT html_content FROM voucher_templates WHERE template_type = 'payment' AND is_default = 1 LIMIT 1");
    $stmt->execute();
...
if (!$voucher) {
    echo "<script>alert('السند غير موجود أو ليس سند صرف'); history.back();</script>";
    exit;
}
...
if ($custom_template) {
    ...
    echo '<!DOCTYPE html><html dir="rtl"><head><title>طباعة سند صرف</title></head><body onload="window.print()">';
    echo $html;
    echo '</body></html>';
...
<title>طباعة سند صرف - <?= htmlspecialchars($voucher['transaction_number']) ?></title>
<style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* رأس السند - سند الصرف يمكن تمييزه بلون مختلف مثلاً أخضر أو أحمر داكن */
    .voucher-header {
        background: linear-gradient(135deg, #2c3e50 0%, #34495e 50%, #465c71 100%);
        color: white;
        padding: 28px 35px;
        position: relative;
        overflow: hidden;
    ...
    @media print {
        ...
        .voucher-header { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    }
...
<div class="print-wrapper" style="position:relative;">
    ...
    <div class="voucher-body">
        ...
        <div class="info-grid">
            ...
            <div class="info-item">
                <div class="label">المستفيد (<?= $entity_label ?>)</div>
                <div class="value">👤 <?= htmlspecialchars($voucher['party_name'] ?? '---') ?></div>
            </div>

            <div class="info-item">
                <div class="label">حساب الصرف</div>
        ...
        <div class="section-title">
            <span>📄</span>
            <span>تفصيل تسديد المشتريات / الفواتير</span>
        </div>
        <table class="invoices-table">
            ...
            <tfoot>
                ...
                <tr>
                    <td colspan="4" style="text-align:right;">المتبقي (رصيد للمستفيد):</td>
        ...
        <div class="signatures">
            ...
            <div class="signature-box">
                <div class="sig-label">المدير المالي / الاعتماد</div>
                <div class="sig-line">...</div>
            </div>
        </div>
    </div>
    ...
    <div class="voucher-footer">
        <span>النظام: <?= htmlspecialchars($company_name) ?></span>
    ...
    <div class="action-buttons">
        ...
        <a href="payments.php" class="btn-back">↩️ رجوع</a>
    </div>
</div>
