...
final class SiteSettingsService
{
    ...
    private function ensureTable(): void
    {
        ...
        $this->database->pdo()->exec("INSERT IGNORE INTO site_settings (id, site_name_ar, tagline_ar, footer_text_ar, password_policy) VALUES (1, 'منصة رحلة', 'احجز رحلتك بسهولة وأمان', '© " . date('Y') . " منصة رحلة', 'medium')");
    }
    ...
    public function publicSettings(): array
    {
        ...
        try {
            $row = $this->database->pdo()->query('SELECT id, site_name_ar, tagline_ar, logo_path, icon_path, footer_text_ar, password_policy FROM site_settings WHERE id = 1 LIMIT 1')->fetch();
    ...
    public function update(array $actor, array $input): array
    {
        ...
        $statement = $this->database->pdo()->prepare(
            'UPDATE site_settings SET site_name_ar = :site_name_ar, tagline_ar = :tagline_ar, footer_text_ar = :footer_text_ar, password_policy = :password_policy, updated_by = :updated_by WHERE id = 1'
        );
        $statement->execute(['site_name_ar' => $siteName, 'tagline_ar' => $tagline !== '' ? $tagline : null, 'footer_text_ar' => $footer, 'password_policy' => $passwordPolicy, 'updated_by' => $actor['id']]);
