...
if ($route === 'tickets' && $method === 'GET') {
    ...
    $tickets = $database->pdo()->prepare(
        "SELECT tk.id, tk.ticket_number, tk.status, tk.qr_token, tk.issued_at, tk.amount, cu.code AS currency_code, cu.symbol_ar AS currency_symbol,
                p.full_name_ar, bs.seat_code, t.trip_number, t.departure_at, co.trade_name AS company_name, co.latitude AS company_latitude, co.longitude AS company_longitude
...
if (preg_match('#^tickets/(\d+)$#', $route, $matches) === 1 && $method === 'GET') {
    ...
    $ticket = $database->pdo()->prepare(
        'SELECT tk.*, b.company_id, b.customer_id, b.agent_id, p.full_name_ar, bs.seat_code, t.trip_number, t.departure_at, co.trade_name AS company_name, co.latitude AS company_latitude, co.longitude AS company_longitude, cu.code AS currency_code, cu.symbol_ar AS currency_symbol
         ...
         INNER JOIN booking_seats bks ON bks.id = tk.booking_seat_id INNER JOIN bus_seats bs ON bs.id = bks.bus_seat_id INNER JOIN trips t ON t.id = b.trip_id INNER JOIN companies co ON co.id = b.company_id INNER JOIN currencies cu ON cu.id = tk.currency_id
