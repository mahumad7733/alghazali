...
final class AdminService
{
    ...
    public function legacyCreateRoute(array $actor, array $input): array
    {
        ...
        return $this->database->transaction(function (PDO $pdo) use ($actor, $companyId, $code, $name, $subrouteIds): array {
            ...
            $ordered = $this->orderSubroutes($items);
            ...
            foreach ($cityIds as $index => $cityId) {
                $station = $this->one($pdo, 'SELECT s.id FROM stations s WHERE s.city_id = :city_id AND s.is_active = 1 ORDER BY s.id LIMIT 1', ['city_id' => $cityId]);
                if ($station === null) {
    ...
    public function createRoute(array $actor, array $input): array
    {
        ...
        try {
            ...
            $pdo->prepare('UPDATE routes SET route_type = :route_type, status = :status WHERE id = :id')->execute(['route_type' => $routeType, 'status' => $status, 'id' => $created['id']]);
    ...
    public function updateRoute(array $actor, int $routeId, array $input): array
    {
        $this->assertPermission($actor, 'manage_routes');
        $pdo = $this->database->pdo();
        $route = $this->one($pdo, 'SELECT id, company_id, code, name_ar, route_type, status FROM routes WHERE id = :id', ['id' => $routeId]);
        ...
        if (mb_strlen($name) < 3) { Response::error('اسم المسار الرئيسي مطلوب.', 'VALIDATION_ERROR', 422); }
        if ($subrouteIds === []) { Response::error('اختر مسارًا فرعيًا واحدًا على الأقل من المسارات الموجودة.', 'VALIDATION_ERROR', 422); }
        $currentIds = array_map(static fn (array $item): int => (int) $item['subroute_id'], $this->all($pdo, 'SELECT subroute_id FROM route_subroute_links WHERE route_id = :route_id ORDER BY stop_order', ['route_id' => $routeId]));
        ...
        if (!$requiresRebuild) {
            $pdo->prepare('UPDATE routes SET name_ar = :name_ar, route_type = :route_type, status = :status WHERE id = :id')->execute(['name_ar' => $name, 'route_type' => $routeType, 'status' => $status, 'id' => $routeId]);
            ...
            return ['id' => $routeId, 'company_id' => (int) $route['company_id'], 'code' => $route['code'], 'name_ar' => $name, 'route_type' => $routeType, 'status' => $status];
        }
        $pdo->prepare('DELETE FROM routes WHERE id = :id')->execute(['id' => $routeId]);
        $rebuilt = $this->legacyCreateRoute($actor, ['company_id' => $companyId, 'code' => $route['code'], 'name_ar' => $name, 'subroute_ids' => $subrouteIds]);
        $pdo->prepare('UPDATE routes SET route_type = :route_type, status = :status WHERE id = :id')->execute(['route_type' => $routeType, 'status' => $status, 'id' => $rebuilt['id']]);
    ...
    public function createTrip(array $actor, array $input): array
    {
        ...
        $busId = filter_var($input['bus_id'] ?? null, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
        ...
        if ($routeId === false || $routeSubrouteId === false || $busId === false || $departure === false || $arrival === false || $arrival <= $departure || mb_strlen($tripNumber) < 3) { Response::error('بيانات الرحلة غير صالحة.', 'VALIDATION_ERROR', 422); }
        return $this->database->transaction(function (PDO $pdo) use ($actor, $routeId, $routeSubrouteId, $busId, $departure, $arrival, $tripNumber): array {
            $route = $this->one($pdo, 'SELECT * FROM routes WHERE id = :id FOR UPDATE', ['id' => $routeId]);
            $bus = $this->one($pdo, 'SELECT * FROM buses WHERE id = :id FOR UPDATE', ['id' => $busId]);
            if ($route === null || $bus === null || (int) $route['company_id'] !== (int) $bus['company_id']) { Response::error('المسار والباص غير متوافقين.', 'VALIDATION_ERROR', 422); }
            $companyId = $this->companyScope($actor, $route['company_id']);
            ...
            $statement = $pdo->prepare('INSERT INTO trips (company_id, route_id, route_subroute_id, bus_id, trip_number, departure_at, arrival_at, booking_open_at, booking_close_at, status, created_by) VALUES (:company_id, :route_id, :route_subroute_id, :bus_id, :trip_number, :departure_at, :arrival_at, NOW(), DATE_SUB(:departure_at_close, INTERVAL 30 MINUTE), \'open\', :created_by)');
            $statement->execute(['company_id' => $companyId, 'route_id' => $routeId, 'route_subroute_id' => $routeSubrouteId, 'bus_id' => $busId, 'trip_number' => $tripNumber, 'departure_at' => $departure->format('Y-m-d H:i:s'), 'arrival_at' => $arrival->format('Y-m-d H:i:s'), 'departure_at_close' => $departure->format('Y-m-d H:i:s'), 'created_by' => $actor['id']]);
            $tripId = (int) $pdo->lastInsertId();
            $pdo->prepare('INSERT INTO trip_seat_inventory (trip_id, bus_seat_id, is_available) SELECT :trip_id, id, 1 FROM bus_seats WHERE bus_id = :bus_id AND is_active = 1')->execute(['trip_id' => $tripId, 'bus_id' => $busId]);
            ...
            return ['id' => $tripId, 'trip_number' => $tripNumber, 'route_subroute_id' => $routeSubrouteId, 'status' => 'open'];
        });
    }
    ...
    public function createRecurringTrips(array $actor, array $input): array
    {
        ...
        return $this->database->transaction(function (PDO $pdo) use ($actor, $template): array {
            ...
            foreach ($template['occurrences'] as $occurrence) {
                if ($this->one($pdo, 'SELECT id FROM trips WHERE bus_id = :bus_id AND departure_at = :departure_at AND status <> \'cancelled\' LIMIT 1 FOR UPDATE', ['bus_id' => $template['bus_id'], 'departure_at' => $occurrence['departure_at']]) !== null) {
            ...
            foreach ($template['occurrences'] as $index => $occurrence) {
                $tripNumber = $template['trip_number_prefix'] . '-' . str_pad((string) ($index + 1), 3, '0', STR_PAD_LEFT);
                $statement = $pdo->prepare('INSERT INTO trips (company_id, route_id, route_subroute_id, bus_id, trip_number, recurrence_group, recurrence_index, departure_at, arrival_at, booking_open_at, booking_close_at, status, created_by) VALUES (:company_id, :route_id, :route_subroute_id, :bus_id, :trip_number, :recurrence_group, :recurrence_index, :departure_at, :arrival_at, NOW(), DATE_SUB(:departure_at_close, INTERVAL 30 MINUTE), \'open\', :created_by)');
                $statement->execute(['company_id' => $companyId, 'route_id' => $template['route_id'], 'route_subroute_id' => $template['route_subroute_id'], 'bus_id' => $template['bus_id'], 'trip_number' => $tripNumber, 'recurrence_group' => $group, 'recurrence_index' => $index + 1, 'departure_at' => $occurrence['departure_at'], 'arrival_at' => $occurrence['arrival_at'], 'departure_at_close' => $occurrence['departure_at'], 'created_by' => $actor['id']]);
                $tripId = (int) $pdo->lastInsertId();
                $pdo->prepare('INSERT INTO trip_seat_inventory (trip_id, bus_seat_id, is_available) SELECT :trip_id, id, 1 FROM bus_seats WHERE bus_id = :bus_id AND is_active = 1')->execute(['trip_id' => $tripId, 'bus_id' => $template['bus_id']]);
                $pdo->prepare('INSERT INTO trip_segment_prices (trip_id, route_segment_id, currency_id, company_amount, amount, source_price_id) SELECT :trip_id, sp.route_segment_id, sp.currency_id, COALESCE(subroute.company_amount, 0), sp.amount, sp.id FROM segment_prices sp INNER JOIN route_segments rs ON rs.id = sp.route_segment_id LEFT JOIN route_subroute_links link ON link.route_segment_id = sp.route_segment_id LEFT JOIN route_subroutes subroute ON subroute.id = link.subroute_id WHERE sp.company_id = :company_id AND rs.route_id = :route_id AND sp.status = \'active\'')->execute(['trip_id' => $tripId, 'company_id' => $companyId, 'route_id' => $template['route_id']]);
    ...
    private function recurringTripTemplate(array $actor, array $input): array
    {
        ...
        if ($routeId === false || $subrouteId === false || $busId === false || $start === false || $end === false || $end < $start || $end > $start->modify('+366 days') || $weekdays === [] || mb_strlen($prefix) < 3) { Response::error('بيانات تكرار الرحلات غير صالحة. اختر فترة لا تتجاوز سنة وأيامًا ورقمًا تمهيديًا صالحًا.', 'VALIDATION_ERROR', 422); }
        $pdo = $this->database->pdo();
        ...
        return ['route_id' => $routeId, 'route_subroute_id' => $subrouteId, 'bus_id' => $busId, 'trip_number_prefix' => $prefix, 'occurrences' => $occurrences];
    }
    ...
    private function bulkTripUpdatePayload(array $actor, array $input): array
    {
        ...
        if ($action === 'status') { $status = in_array($input['status'] ?? null, ['scheduled', 'open', 'completed', 'cancelled', 'expired'], true) ? (string) $input['status'] : null; $label = 'تغيير الحالة'; if ($status === null) { Response::error('اختر حالة صالحة للرحلات.', 'VALIDATION_ERROR', 422); } }
    ...
    private function tripsForBulkUpdate(PDO $pdo, array $tripIds, bool $forUpdate): array
    {
        ...
        $statement = $pdo->prepare('SELECT t.id, t.company_id, t.trip_number, t.departure_at, (SELECT COUNT(*) FROM bookings b WHERE b.trip_id = t.id) AS booking_count FROM trips t WHERE t.id IN (' . $holders . ')' . ($forUpdate ? ' FOR UPDATE' : ''));
    ...
    public function updateTrip(array $actor, int $tripId, array $input): array
    {
        $this->assertPermission($actor, 'manage_trips');
        $pdo = $this->database->pdo();
        $trip = $this->one($pdo, 'SELECT id, company_id FROM trips WHERE id = :id', ['id' => $tripId]);
        ...
        $departure = \DateTimeImmutable::createFromFormat('Y-m-d H:i:s', (string) ($input['departure_at'] ?? ''));
        $arrival = \DateTimeImmutable::createFromFormat('Y-m-d H:i:s', (string) ($input['arrival_at'] ?? ''));
        if (mb_strlen($number) < 3 || $departure === false || $arrival === false || $arrival <= $departure) { Response::error('بيانات الرحلة غير صالحة.', 'VALIDATION_ERROR', 422); }
        $pdo->prepare('UPDATE trips SET trip_number = :trip_number, departure_at = :departure_at, arrival_at = :arrival_at, booking_close_at = DATE_SUB(:departure_close, INTERVAL 30 MINUTE) WHERE id = :id')->execute(['trip_number' => $number, 'departure_at' => $departure->format('Y-m-d H:i:s'), 'arrival_at' => $arrival->format('Y-m-d H:i:s'), 'departure_close' => $departure->format('Y-m-d H:i:s'), 'id' => $tripId]);
        $this->audit->log((int) $actor['id'], (int) $companyId, 'trip_updated', 'trip', $tripId, null, ['trip_number' => $number]);
        return ['id' => $tripId, 'trip_number' => $number];
    }
    ...
    {
        $this->assertPermission($actor, 'manage_trips');
        $status = in_array($input['status'] ?? null, ['scheduled', 'open', 'completed', 'cancelled', 'expired'], true) ? (string) $input['status'] : '';
        if ($status === '') { Response::error('حالة الرحلة غير صالحة.', 'VALIDATION_ERROR', 422); }
        $trip = $this->one($this->database->pdo(), 'SELECT company_id FROM trips WHERE id = :id', ['id' => $tripId]);
        if ($trip === null) { Response::error('الرحلة غير موجودة.', 'NOT_FOUND', 404); }
        $companyId = $this->companyScope($actor, $trip['company_id']);
        $this->database->pdo()->prepare('UPDATE trips SET status = :status WHERE id = :id')->execute(['status' => $status, 'id' => $tripId]);
        $this->audit->log((int) $actor['id'], (int) $companyId, 'trip_status_updated', 'trip', $tripId, null, ['status' => $status]);
        return ['id' => $tripId, 'status' => $status];
    }
