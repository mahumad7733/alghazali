...
final class DashboardService
{
    ...
    public function overview(array $actor, array $filters = []): array
    {
        ...
        $tripPeriodParams = $tripParams + ['trip_start' => $start, 'trip_end' => $end];
        $tripStatement = $pdo->prepare("SELECT COUNT(*) AS total_trips,
            SUM(t.status = 'open') AS open_trips, SUM(t.status = 'completed') AS completed_trips, SUM(t.status = 'cancelled') AS cancelled_trips
            FROM trips t WHERE {$tripWhere} AND t.departure_at >= :trip_start AND t.departure_at < :trip_end");
