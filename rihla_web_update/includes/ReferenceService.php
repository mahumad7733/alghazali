...
final class ReferenceService
{
    ...
    public function searchTrips(int $originCityId, int $destinationCityId, string $date, string $busType = ''): array
    {
        ...
        $statement = $this->database->pdo()->prepare(
            "SELECT t.id, t.trip_number, t.bus_id, t.trip_type, t.seat_count, t.departure_at, t.arrival_at, t.status, r.route_type, r.journey_type, co.id AS company_id, co.trade_name AS company_name, co.latitude AS company_latitude, co.longitude AS company_longitude, co.logo_path, co.cover_image_path,
                    ...
                    rs.id AS segment_id, oc.name_ar AS origin_name, dc.name_ar AS destination_name, os.name_ar AS origin_station_name, os.address AS origin_station_address, ds.name_ar AS destination_station_name, ds.address AS destination_station_address,
                    tsp.amount, cu.code AS currency_code, cu.symbol_ar AS currency_symbol,
                    CASE WHEN EXISTS (SELECT 1 FROM trip_seat_inventory inventory WHERE inventory.trip_id = t.id) THEN LEAST(COALESCE(NULLIF(t.seat_count, 0), b.seat_count, 0), (SELECT COUNT(*) FROM trip_seat_inventory ti WHERE ti.trip_id = t.id AND ti.is_available = 1)) ELSE COALESCE(NULLIF(t.seat_count, 0), b.seat_count, 0) END AS available_seats,
             ...
             WHERE t.status = \'open\' AND t.departure_at >= NOW() AND DATE(t.departure_at) = :travel_date AND os.city_id = :origin_city_id AND ds.city_id = :destination_city_id
    ...
    public function trip(int $tripId): array
    {
        ...
        $trip['stops'] = $this->all(
            'SELECT rs.id, rs.stop_order, s.name_ar AS station_name, s.address AS station_address, c.name_ar AS city_name, rs.arrival_offset_minutes, rs.departure_offset_minutes
        ...
        $trip['segments'] = $this->all(
            'SELECT seg.id, seg.origin_order, seg.destination_order, ost.name_ar AS origin_station, ost.address AS origin_station_address, dst.name_ar AS destination_station, dst.address AS destination_station_address, tsp.amount, cu.code AS currency_code, cu.symbol_ar AS currency_symbol
