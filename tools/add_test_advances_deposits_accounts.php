...
echo "<h1>Adding Test Employee Advances & Deposits Accounts under 11301 السلف والعهد</h1>";

try {
    $pdo->beginTransaction();
    
    $id_11301 = $pdo->query("SELECT id FROM unified_accounts WHERE account_code = '11301'")->fetchColumn();
    if (!$id_11301) {
        throw new Exception("Account 11301 (السلف والعهد) not found! Run fix_chart_of_accounts.php first!");
    }
    echo "<p>Found 11301 (السلف والعهد) with ID: $id_11301</p>";
    
    $testAccounts = [
        ['11301001', 'سلف الموظفين'],
        ['11301002', 'عهد الموظفين'],
        ['11301003', 'سلف المندوبين']
    ];
    ...
    foreach ($testAccounts as $acc) {
        ...
        if (!$checkStmt->fetch()) {
            ...
            ");
            $insertStmt->execute([$code, $name, $id_11301]);
