-- ==========================================================
-- PATCH: إصلاح خطأ sp_create_invoice — النسخة النهائية والمصححة 100%
-- آخر تحديث: 31 يوليو 2026
-- ==========================================================
-- قائمة الإصلاحات المطبقة في هذا الملف:
--
-- 🔧 1. ترتيب البارامتر رقم 8: تم تصحيحه إلى p_branch_entity_id
--       (كان p_service_id خاطئاً وكان يختلط ببيانات الجهة الفرعية)
--
-- 🔧 2. إضافة عمود branch_entity_id إلى INSERT في جدول invoices
--       (لم يكن في الباتش الأصلي على الإطلاق)
--
-- 🔧 3. إصلاح أعمدة جدول account_balances_unified (12 عمود صحيح):
--       - تم إزالة الأعمدة غير الموجودة أصلاً: (balance, debit_total, credit_total, debit_count, credit_count)
--       - تم استخدام نفس الأعمدة من الإجراء الرسمي sp_ensure_opening_balance
--
-- 🔧 4. استرجاع currency_code من جدول currencies قبل إدراج الرصيد
--
-- 🔧 5. إضافة branch_entity_id إلى سجل audit_logs للفاتورة
--
-- 🔧 6. إصلاح مشكلة تعارض Collation (utf8mb4_unicode_ci ⇄ utf8mb4_general_ci)
--       عبر SET NAMES صريحة في بداية الإجراء
-- ==========================================================
...
sp_create_invoice_body:BEGIN
    ...
    -- ====== إصلاح مشكلة تعارض الـ Collation ======
    ...
    -- ====== FIX: إدراج عملة الفاتورة تلقائياً في account_balances_unified ======
